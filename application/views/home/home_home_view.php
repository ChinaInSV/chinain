<?php
	...
	$promo=0;
	$duo=0;
	if($dias[date("w")]=="lunes" || $dias[date("w")]=="martes" || $dias[date("w")]=="miercoles" || $dias[date("w")]=="jueves" || $dias[date("w")]=="viernes"):
		$promo=0;
		$duo=1;
	endif;
?>	
					...
					<div class="panel">
						<div class="panel-body">
							...
							<div class="row bord-btm pad-btm">
								...
								<div class="col-sm-4 text-left bord-rgt" id="nueva-orden-menu-mode-view-wrapper">
								...
								<!--Menus-->
								<div class="col-sm-7 text-left nueva-orden-togglesbtn nueva-orden-changemenu" id="nueva-orden-menus-wrapper">
									MENU: 
									<?php 
										if(count($menus)):
											$count_menu=0;
							...
							<div class="row">
								<div id="nueva-orden-menu-wrapper" class="mar-top" style="height:450px;">
									...
									<div class="row" id="nueva-orden-modo-menu-wrapper" <?php if($config->menu_mode->value!=1):?>style="display:none;"<?php endif;?>>
										<!--Categorias del menu-->
										<div class="col-sm-3 bord-all bg-gray-light">
													...
													if(count($menus)):
														foreach($menus as $cmenu):
															if(count($cmenu->categorias)):
																foreach($cmenu->categorias as $categoria):$hidden_cat="";if($duo==0 && $categoria->id == 20):$hidden_cat="hidden";endif;
												?>												
												<button type="button"  style="white-space:normal;height:64px;" data-toggle="button" class="<?php echo $hidden_cat;?> btn-cat-menu btn btn-block btn-lg btn-primary btn-active-mint mar-btm" <?php if($cmenu->id!=$id_sel_menu):?>style="display:none;"<?php endif;?> data-menu="<?php echo $cmenu->id;?>" data-id="<?php echo $categoria->id;?>"><?php echo $categoria->nombre;?></button>
												<?php  
																endforeach;
															endif;
														endforeach;
										...
										<div class="col-sm-9">
											...
											<div class="form-group pad-btm bord-btm">
											...
											<div class="row">
												<div class="mar-ver" id="nueva-orden-platos-wrapper" style="overflow-y:auto;height:478px;">
													...
													<div class="col-sm-4">
														...
														<div class="mar-btm btn-plato" style="display:none;">
															<button type="button" class="btn btn-block btn-lg btn-default " style="height:90px;background-color:rgb(211,211,211);color:rgb(48,48,48);white-space:normal;word-break:break-all;" data-categoria="<?php echo $cplatos->categoria;?>" data-id="<?php echo $cplatos->id;?>" data-acompanamientos='<?php if(count($cplatos->acompanamientos)):echo json_encode($cplatos->acompanamientos);endif;?>' data-notas='<?php echo json_encode($cplatos->notas);?>' data-precio="<?php echo number_format($cplatos->precio,$config->precios_decimal_precision->value);?>"  style="white-space:normal;"><?php echo $cplatos->nombre;?></button>
														</div>
														<?php 
																			endforeach;
																		endif;
									...
									<input id="nueva-orden-servicio" type="hidden" value="<?php if(isset($orden->servicio))echo $orden->servicio;?>">
									<input id="nueva-orden-cliente" type="hidden" value="<?php if(isset($orden->cliente))echo $orden->cliente;?>">
									<input id="nueva-orden-direccion" type="hidden" value="<?php if(isset($orden->direccion))echo $orden->direccion;?>">
									<input id="nueva-orden-telefono" type="hidden" value="<?php if(isset($orden->telefono))echo $orden->telefono;?>">
									<input id="nueva-orden-formapago" type="hidden" value="<?php if(isset($orden->formapago))echo $orden->formapago;?>">
									<input id="nueva-orden-origen" type="hidden" value="<?php if(isset($orden->origen)){echo $orden->origen;}else{echo "0";}?>">
								<!--/Ajax Load-->
								</div>
							</div>
						</div>
								...
								<div class="col-md-10">
									...
									<div style="overflow-y:auto;height:335px;" class="bord-all" id="nueva-orden-platos-tabla-wrapper">
										<table class="table table-hover table-vcenter table-striped" id="nueva-orden-platos-tabla">
											...
											?>
											<tr id="" class="not-active" data-id="<?php echo $plato->id;?>" data-cant="<?php echo number_format($plato->cant,$config->cant_decimal_precision->value);?>" data-nombre="<?php echo $plato->nombre;?>" data-precio="<?php echo number_format($plato->precio,$config->precios_decimal_precision->value);?>"  data-total="<?php echo number_format(($plato->precio*$plato->cant),$config->precios_decimal_precision->value);?>" data-acompanamientos="" data-acompanamientosstr="" data-prenotas="" data-notas="<?php $plato->notas?>" data-enviado="1" data-cantenviados="<?php echo number_format($plato->cant,$config->cant_decimal_precision->value);?>">
												...
												<td class="plato-precio-field text-center " width="15%"><?php echo number_format($plato->precio,$config->precios_decimal_precision->value);?></td>
												<td class="plato-total-field text-center " width="20%"><?php echo number_format(($plato->precio*$plato->cant),$config->precios_decimal_precision->value);?></td>
									...
									<div class="row mar-top bord-top pad-top text-center">
										<button id="nueva-orden-cobrar-btn" class="btn btn-lg btn-info" style="padding:16px 24px">COBRAR</button>
									</div>
				...
				<div class="modal-dialog modal-md">
					<div class="modal-content">
						...
						<div class="modal-body bg-gray">
							...
							<!--POS-->
							<div id="facturacion-app-venta-cobro-pos-wrapper" class="row">
								<span class="col-sm-5 text-right text-main text-2x">POS</span>
								<div class="col-sm-1 text-right">
									<p class="text-main text-2x">$</p>
								</div>
								<div class="col-sm-4 text-right">
									<p id="facturacion-app-venta-cobro-pos"  class="text-main text-2x"></p>
								</div>
							</div>
							<!--Efectivo-->
